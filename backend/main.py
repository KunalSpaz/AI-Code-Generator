...
EXPLAIN_PROMPT = """Analyze and explain this {language} code snippet by snippet:
...
## Code Overview
[Brief summary of what the entire code does]

## Snippet-by-Snippet Explanation
[Break down the code into logical sections/snippets and explain each one]

## Key Concepts
[Highlight important programming concepts, algorithms, or patterns used]

## Usage Notes
[Any important usage considerations or potential improvements]

Focus on explaining the logic and purpose of each part rather than rewriting the code."""

DEBUG_PROMPT = """Analyze this {language} code for errors and provide debugging assistance:
...
## Error Analysis
[Identify and explain any errors, bugs, or issues in the code]

## Root Cause
[Explain why these errors occur and what causes them]
...
If no errors are found, explain what the code does and suggest potential improvements."""
...
async def generate_code(query: str, language: str, task_type: str = "generate") -> str:
    if task_type == "debug":
        return await debug_code(query, language)
    elif task_type == "explain":
        return await explain_code(query, language)
    else:
...
async def chat(request: ChatRequest) -> ChatResponse:
    try:
        ...
        if task_type == "debug":
            debug_analysis = await generate_code(request.message, language, task_type)
            return ChatResponse(
                conversation_id=conversation_id,
                message=request.message,
                code=debug_analysis,
                complexity="Debug Analysis - See code section for details",
                docs="Debugging assistance provided with error identification and solutions",
                language=language
            )
        elif task_type == "explain":
            explanation = await generate_code(request.message, language, task_type)
            return ChatResponse(
                conversation_id=conversation_id,
                message=request.message,
                code=explanation,
                complexity="Code Explanation - See code section for details",
                docs="Snippet-wise code explanation provided",
                language=language
            )
        else:
            code = await generate_code(request.message, language, task_type)
